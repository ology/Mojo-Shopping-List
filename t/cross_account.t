...
$t->post_ok('/a/update_list' => form => { list => 2, name => 'Test List!' })
  ->status_is(200)
  ->content_like(qr/Invalid fields/);

# Test that the wrong list cannot be viewed
$t->get_ok('/a/view_list?list=2')
  ->status_is(200)
  ->content_like(qr/Invalid fields/);
...
$t->post_ok('/a/update_item' => form => { active => 0, list => 1, sort => 'alpha', item => 2, name => 'Test Item!' })
  ->status_is(200)
  ->content_like(qr/Invalid fields/);

# Test that an item cannot be updated on the wrong list
$t->post_ok('/a/update_item' => form => { active => 0, list => 2, sort => 'alpha', item => 1, name => 'Test Item!' })
  ->status_is(200)
  ->content_like(qr/Invalid fields/);
...
$t->get_ok('/a/delete_item?list=2&sort=alpha&item=1')
  ->status_is(200)
  ->content_like(qr/Invalid fields/);

# Test that the wrong list cannot be deleted
$t->get_ok('/a/delete_list?list=2')
  ->status_is(200)
  ->content_like(qr/Invalid fields/);
